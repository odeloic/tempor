...
import { timerState, sessions, type TimerState } from '@/db/schema';
...
export function useTimer(): UseTimerReturn {
  ...
  const start = useCallback(async (projectId: string) => {
    // If timer is running on another project, save that session first
    if (state.status === 'running' && state.projectId && state.projectId !== projectId) {
      const lastResumedMs = state.lastResumedAt?.getTime() ?? null;
